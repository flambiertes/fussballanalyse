...
for start in [2018,2022]:
 x=pd.read_csv(f'data/backtest_d1_{start}_{start+3}_official_frozen.csv')
 x=x.drop(columns='matchday',errors='ignore')
 ...
 for i,r in enumerate(y.itertuples()):
  ...
  tips={
   'odds_fixed_2_1':[(2,1),(1,1),(1,2)][favourite] if r.bookmaker_weight>0 else (r.tip_home,r.tip_away),
   'odds_fixed_1_0':[(1,0),(1,1),(0,1)][favourite] if r.bookmaker_weight>0 else (r.tip_home,r.tip_away),
 ...
 grouped.to_csv(f'data/frozen_round_audit_{start}_{start+3}.csv')
 y.drop(columns='_merge').to_csv(f'data/frozen_predictions_{start}_{start+3}.csv',index=False)
Path('data/frozen_round_model_audit.json').write_text(json.dumps(reports,indent=2),encoding='utf-8')
